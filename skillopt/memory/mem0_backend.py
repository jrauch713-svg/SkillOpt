...
class SkillMemory:
    ...
    def _search(self, query: str, limit: int) -> list[dict]:
        # Mem0 2.x search contract: entity scoping goes in `filters`, and the
        # result count is `top_k`. Passing a top-level `user_id` raises
        # ValueError before any request is made, which `_bounded` would turn
        # into a silent empty result — writes would keep succeeding while
        # retrieval never returned anything. `add()` still takes `user_id` at
        # the top level, so the two calls differ on purpose.
