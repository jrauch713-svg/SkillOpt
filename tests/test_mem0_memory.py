...
class _StrictMem0V2Client:
    """A stand-in that enforces the real Mem0 2.x ``search`` signature.

    ``FakeClient`` accepts arbitrary keyword arguments, so it happily swallowed
    the 0.1.x call shape (``user_id=`` / ``limit=``) and every test passed while
    real Mem0 2.x raised ``ValueError`` before sending a request. ``_bounded``
    then converted that into a silent empty result, so writes kept succeeding
    while retrieval returned nothing. This double makes that regression fail
    loudly instead.

    Contract mirrored from mem0 2.0.x: ``search(query, *, filters=None,
    top_k=None)``; ``add()`` still accepts a top-level ``user_id``.

    Thanks to @WODE25500, who identified the mismatch and supplied this shape.
    """
...
# ── 10. Mem0 API contract ─────────────────────────────────────────────────────

def test_search_uses_the_mem0_v2_contract():
    """Retrieval must call Mem0 2.x's actual search signature.

    Mem0 2.x rejects a top-level ``user_id``/``limit`` in ``search()`` with a
    ``ValueError`` raised *before* any network call, requiring
    ``filters={"user_id": ...}`` and ``top_k=``. Because ``_bounded`` degrades
    failures to ``[]``, that mismatch is silent: writes keep succeeding while
    retrieval quietly returns nothing. Hence a signature-strict double rather
    than the permissive one, which cannot catch this.
    """
    memo = "Open the drawer before searching it."
    settings = _enabled_settings()
    client = _StrictMem0V2Client(search_results=[{"memory": memo}])
    memory = SkillMemory(settings, client=client)

    ctx = hook_pre_reflect(memory, "current skill text", "prior step context")

    assert client.searches, "search was never called against the strict client"
    _query, filters, top_k = client.searches[0]
    assert filters == {"user_id": settings.namespace}, (
        f"scoping must go through filters, got {filters!r}"
    )
    assert top_k == settings.retrieval_limit, f"expected top_k, got {top_k!r}"
    assert memo in ctx, "retrieval produced nothing under the 2.x contract"
    memory.close()
    print("ok  10. search uses the Mem0 2.x contract (filters/top_k)")


def test_write_path_still_uses_top_level_user_id():
    """``add()`` keeps a top-level ``user_id`` in 2.x — the two calls differ."""
    settings = _enabled_settings()
    client = _StrictMem0V2Client()
    memory = SkillMemory(settings, client=client)
    memory.store_skill_iteration(epoch=0, step=0, skill_text="s", score=1.0)

    assert client.adds, "write never reached the client"
    _messages, kwargs = client.adds[0]
    assert kwargs.get("user_id") == settings.namespace
    memory.close()
    print("ok  10b. add() still scopes with a top-level user_id")
...
ALL_TESTS = [
    ...
    test_search_uses_the_mem0_v2_contract,
    test_write_path_still_uses_top_level_user_id,
